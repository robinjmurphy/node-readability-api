...
var ERROR_CODES = [400, 401, 403, 404, 500];
...
function handleHttpResponse(err, res, body, cb) {
  ...
  if (ERROR_CODES.indexOf(res.statusCode) !== -1) {
    ...
    if (body.messages) message = body.messages.join(' ');

    return cb(new Error('HTTP ' + res.statusCode + ': ' + message));
  }
...
Reader.prototype.request = function (method, path, options, cb) {
  ...
  if (!config.containsReaderCredentials()) {
    cb(new Error('The API must be configured with a set of developer keys before it can be used'));
  }
